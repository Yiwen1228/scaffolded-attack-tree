...
// Synonym library for flexible node matching
const SYNONYMS = {
...
const SCENARIOS = [
  ...
  },
  {
    ...
    referenceNodes: [
      "Compromise Hospital System", "Steal Credentials", "SQL Injection",
      "Access Patient Records", "Modify Patient Records", "Ransomware Attack",
      "Insider Threat", "Denial of Service", "Network Intrusion"
    ],
    exampleTree: [
  ...
  },
  {
    ...
    description: "An e-commerce platform allows customers to browse products, make purchases using credit cards, and leave reviews. The platform stores customer payment details, order histories, and personal addresses. Sellers can also list products and manage inventory.",
    scaffolded: true,
    hints: [
    ...
    referenceNodes: [
      "Compromise E-commerce Platform", "Steal Payment Info", "SQL Injection",
      "Account Takeover", "Session Hijacking", "Fake Reviews",
      "Price Manipulation", "Denial of Service", "Phishing Attack"
    ],
    exampleTree: [
  ...
  },
  {
    ...
    description: "A smart home IoT device (such as a smart speaker or security camera) connects to the home Wi-Fi network and is controlled via a mobile app. It records audio and video and sends data to a cloud server.",
    scaffolded: false,
    hints: [],
    suggestions: [],
    referenceNodes: [
      "Compromise Smart Home Device", "Steal Credentials", "Network Intrusion",
      "Control Device", "Eavesdrop", "Disable Device",
      "Firmware Attack", "Steal Personal Data", "Denial of Service"
    ],
    exampleTree: [
  ...
  const renderNode = (nodeId, depth = 0) => {
    ...
    return (
      <div key={nodeId} style={{ marginLeft: depth * 20, marginTop: 4 }}>
        ...
        {children.length > 0 && (
          <div style={{ borderLeft: '2px solid #334155', marginLeft: 12, paddingLeft: 6 }}>
...
function TreeBuilder({ scenario, onComplete }) {
    ...
    return (
      <div key={nodeId} style={{ marginLeft: depth * 22, marginTop: 5 }}>
        ...
        <div onClick={() => setSelectedNode(nodeId)} style={{
          ...
          {!node.isRoot && (
            <button onClick={(e) => { e.stopPropagation(); deleteNode(nodeId); }}
              style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', fontSize: 14 }}>✕</button>
          )}
        </div>
        {children.length > 0 && (
          <div style={{ borderLeft: '2px solid #334155', marginLeft: 14, paddingLeft: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          <div style={{ background: '#1e293b', borderRadius: 10, padding: 14, marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
              <button onClick={() => setNewNodeLogic('OR')}
                style={{ flex: 1, padding: '5px', borderRadius: 5, border: 'none', background: newNodeLogic === 'OR' ? '#0891b2' : '#334155', color: 'white', cursor: 'pointer', fontSize: 12 }}>OR</button>
              <button onClick={() => setNewNodeLogic('AND')}
                style={{ flex: 1, padding: '5px', borderRadius: 5, border: 'none', background: newNodeLogic === 'AND' ? '#7c3aed' : '#334155', color: 'white', cursor: 'pointer', fontSize: 12 }}>AND</button>
            </div>
          ...
          <div style={{ background: '#1e293b', borderRadius: 10, padding: 14, marginBottom: 12 }}>
            ...
            {scenario.scaffolded && (
              <p style={{ color: '#64748b', fontSize: 10, marginTop: 4 }}>Scaffolding reduces at 30% and 60%</p>
            )}
          </div>
        ...
        <div style={{ marginTop: 20, background: '#1e293b', borderRadius: 10, padding: 20 }}>
          ...
          <div style={{ background: '#0f172a', borderRadius: 8, padding: 12, marginBottom: 16 }}>
            ...
            {evaluation.matchedNodes.length > 0 && (
              <p style={{ color: '#94a3b8', fontSize: 12, marginTop: 4 }}>Matched: {evaluation.matchedNodes.join(', ')}</p>
            )}
          </div>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
...
function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white', fontFamily: 'sans-serif', padding: 24 }}>
      <h1 style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 4 }}>🌳 Scaffolded Attack Tree Builder</h1>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {SCENARIOS.map((s, i) => (
          <div key={s.id} style={{ flex: 1, minWidth: 100, padding: '6px 8px', borderRadius: 6, fontSize: 11, textAlign: 'center',
            background: i < currentScenario ? '#166534' : i === currentScenario ? '#2563eb' : '#1e293b',
            color: i < currentScenario ? '#6ee7b7' : 'white',
            border: i === currentScenario ? '2px solid #60a5fa' : '2px solid transparent' }}>
            {i < currentScenario ? '✅ ' : `${i + 1}. `}{s.title.split(' ').slice(0, 2).join(' ')}
      ...
      {!showQuestionnaire && !sessionComplete && (
        <>
          ...
          <TreeBuilder key={currentScenario} scenario={scenario} onComplete={handleScenarioComplete} />
        </>
      )}
        ...
        <div style={{ background: '#1e293b', borderRadius: 10, padding: 24 }}>
          ...
          {results.map((r, i) => (
            <div key={i} style={{ background: '#0f172a', borderRadius: 8, padding: 12, marginBottom: 10 }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: 13 }}>
                Completeness: <strong style={{ color: '#34d399' }}>{r.percentage}%</strong> ({r.matched}/{r.possible} nodes matched)
              </p>
